...
<body>
  ...
  <script>
    ...
    // Download Pass as Image -> replace with PDF export
    document.getElementById('downloadBtn').addEventListener('click', async function() {
      try {
        // render at higher resolution
        const canvas = await html2canvas(passOutput, { scale: 2 });
        ...
        const pxToMm = usableWidth / imgWidthPx;
        ...
        if (imgHeightMm <= pageHeightMm) {
          ...
          const ratio = img.width / usableWidth; // px per mm scale approximation
          const sliceHeightPx = Math.floor(pageHeightMm * ratio);
          let y = 0;
          while (y < img.height) {
            ...
            ctx.drawImage(img, 0, y, img.width, h, 0, 0, canvasPage.width, canvasPage.height);
            ...
            const pageImgHeightMm = (pageImgProps.height * usableWidth) / pageImgProps.width;
            pdf.addImage(pageData, 'PNG', margin, margin, usableWidth, pageImgHeightMm);
            y += sliceHeightPx;
            if (y < img.height) pdf.addPage();
          }
        }
