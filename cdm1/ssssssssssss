...
body{
  ...
  background: linear-gradient(135deg, #ececeeff 0%, #fefdfeff 100%);
...
.card-pastel{
  background:linear-gradient(120deg,var(--),var(--blue));
...
<div class="sidebar" id="sidebar">
  ...
  <a href="#" data-section="trackbus"><i class="fas fa-map-marker-alt"></i> Track Bus</a>
...
// 🗺️ Load Bus Location on Track Bus Click
...
document.querySelector('[data-section="trackbus"]').addEventListener('click', loadBusLocation);

// Replace the existing map script with this improved version
const mapElement = document.getElementById('map');
let busMap = null;
let busMarker = null;

function initializeMap(lat = 12.9716, lng = 77.5946) { // Default coords for Bangalore
    if (!busMap) {
        busMap = L.map('map').setView([lat, lng], 13);
...
function updateBusLocation() {
    fetch('get_bus_location.php?driver_id=<?= $driver['id'] ?? 0 ?>')
        .then(response => {
        ...
        .then(data => {
            ...
            const map = initializeMap(data.latitude, data.longitude);
            ...
            if (busMarker) {
                ...
                busMarker = L.marker([data.latitude, data.longitude])
                    .addTo(map)
                    .bindPopup(`Bus ${data.bus_no || ''}<br>Last updated: ${new Date().toLocaleTimeString()}`);
            }
        ...
        .catch(error => {
            mapElement.innerHTML = `
                <div class="alert alert-warning">
                    ...
                    Unable to track bus: ${error.message}
                </div>`;
        });
}

// Update location every 30 seconds when tracking section is active
let locationInterval = null;

document.querySelector('[data-section="trackbus"]').addEventListener('click', function() {
    updateBusLocation();
    locationInterval = setInterval(updateBusLocation, 30000);
});

// Clear interval when switching to other sections
document.querySelectorAll('.sidebar a:not([data-section="trackbus"])').forEach(link => {
