...
<body>
    ...
    <!-- Action buttons container (Track and Verify) -->
    <div class="action-buttons">
        <!-- Track Button -->
        <form action="bus_tracking.php" method="GET">
            <button type="submit" class="btn btn-outline-dark">TracKMyBus</button>
                ...
                <!-- Verify USN Button -->
        <form action="verification.php" method="POST" class="verify-form">
            <button type="submit" class="btn verify-btn">Verify USN</button>
        </form>
    </div>

    <!-- Track button with icon (alternative style) -->
    ...
    <!-- Banner -->
    <div class="banner">
    <span> Welcome to GSSSIETW Bus Management System  Developed by @PriyaSingh 
        Department of Computer Science in Artificial Intaligence and Mechine learning</span>
    </div>
    ...
    <div class="login-box">
      ...
      <!-- Right-side vertical route buttons -->
    <div class="route-sidebar">
      <h4 class="text-center mb-3">Select Your Bus Route</h4>
      <div class="d-flex flex-column align-items-end gap-2">
        <?php for ($i = 1; $i <= 17; $i++): ?>
          <a href="route_details.php?route=<?php echo $i; ?>" class="btn btn-<?php echo ($i == 1) ? 'warning' : 'primary'; ?>">
            Route No:<?php echo str_pad($i, 2, '0', STR_PAD_LEFT); ?>
          </a>
        <?php endfor; ?>
      </div>
    </div>
    <script>
        document.querySelector('.verify-form').addEventListener('submit', function(e){
            ...
            setTimeout(() => {
                this.submit(); // continue to verification.php after fade
            }, 600);
        });
    </script>
